function [As, Ad, Asn] = Lap3dLocCorrectionOnly(s,ord)
...
%
% This version does not include the dense contribution from the
% punctured trapezoidal rule.
%
% [As,Ad,Asn] = Lap3dLocCorrectionOnly(s,ord)
...
Ad = sparse(s.N, s.N);
Asn = sparse(s.N, s.N);
% NB: For SLP, don't include jacobian
% (Lap3dSLPmat was called with if_jac=0)
% However, the jacobian is added at the end of this function,
% so it should be fine for the user to include the jacobian
% in the punctured trapezoidal rule code.
